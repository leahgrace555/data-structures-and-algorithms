...
const createServer = () => {
  ...
  app.get('/hello', (request,response) => {
    response.status(200).send('hello, I am a human.')
  })

  app.get('/aboutme', (request,response) => {
    response.status(200).send('I am definitely a human. I like bunnies and kittens. That is all')
  })

  app.get('/favoritefoods', (request,response) => {
    response.status(200).send(['pizza','icecream','tacos','pina coladas'])
  })

  app.get('*', (request,response) => {
    response.status(404).send('whoops. Not found');
  })
...
const divisibleByFiveTwoToThePower = (input) => {
  let finalArr = []
  input.map (array => {
    let divByFiveTwoToThe = []
    array.map(value => {
      if(value %5 ===0){
        value = Math.pow(2,value);
        divByFiveTwoToThe.push(value);
      } else{ console.log('ha nope')}
    })
    finalArr.push(divByFiveTwoToThe);
  }); return finalArr
};
